...
public class Viaje {
	...
	public Viaje(String codViaje,String nombre, int horaSalida, int horaRegreso, int numParadas) 
		...
		this.paradas = new Parada[this.numParadas];
		this.contParadas = 0;
	}
	...
	public double coste(){
		double precio = 0;
		precio = (this.horaRegreso - this.horaSalida) * 50 + this.contParadas * 10;
		if(!this.paradasVacio())
			for(int i=0; i < this.contParadas; ++i) {
				if(this.paradas[i] instanceof ParadaActividad)
					precio += ((ParadaActividad)this.paradas[i]).getPrecioActividad();
			}
